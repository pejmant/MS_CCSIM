...
T = 80;   % Template size which should be even
...
mrp = 0; % Multiple random path flag (1:on, 0:off). It is strongly suggested to on it option for conditional simulation.
ms_level = 3; % Number of MS level (1, 2, 3).
prop = 0.4;   % The proportion of TI which will be scanned to find the matched hard data (0 < prop <=1)). Larger cofficient helps the hard data to honred, but it may cause some discountinuties.
...
C_2D = CCSIM_2D(ti, hd, T, OL, CT, fc, prop, rad, cand, mrp, ms_level, real_numb);

%% 3D   S I M U L A T I O N    E X A M P L E

% Input Parameters

load ti_3D_3f;   %  Two examples are avilable: ti_3D_3f and ti_3D_2f
hd = NaN(180,140,30); % Simulation grid (1000x1000)
CT = [2 2 2];   % Co-Template size in 'i' and 'j' directions
OL = [12 12 3];   % Overlap size which should be even
...
C_3D = CCSIM_3D(ti, hd, T, OL, CT, fc, prop, rad, cand, mrp, ms_level, real_numb);
